...
async function loadData() {
  ...
  $("#total-records").textContent = codes.totalRecords.toLocaleString("ar-EG-u-nu-latn");
  ...
  $("#footer-total").textContent = codes.totalRecords.toLocaleString("ar-EG-u-nu-latn");
  renderCategoryChips();
...
function renderResults(query) {
  ...
  $("#result-count").textContent = results.length.toLocaleString("ar-EG-u-nu-latn");
...
/* ===================== آخر الملفات التي تم تحليلها ===================== */
function escapeAttr(str) { return String(str).replace(/"/g, "&quot;"); }

async function renderRecentFiles() {
  const wrap = $("#recent-files-list");
  if (!wrap) return;
  if (!window.IscoreHistoryDB) {
    wrap.innerHTML = `<div class="empty-state" style="padding:24px"><p>تعذر تحميل وحدة السجل المحلي.</p></div>`;
    return;
  }
  try {
    const items = await IscoreHistoryDB.getAll(6);
    if (!items.length) {
      wrap.innerHTML = `
        <div class="empty-state" style="padding:30px 20px">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 16V4M12 4l-4 4M12 4l4 4" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 16v3a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-3"/></svg>
          <h3>لا توجد ملفات محللة بعد</h3>
          <p>ابدأ بتحليل أول تقرير آيسكور وسيظهر هنا تلقائيًا.</p>
        </div>`;
      return;
    }
    const statusLabel = { ok: "مكتمل", partial: "جزئي", error: "خطأ" };
    const statusColor = { ok: "#1e8f5f", partial: "#c07a12", error: "#b23b3b" };
    wrap.innerHTML = items.map((it) => `
      <a class="recent-file-card" href="tools/extractor.html?id=${encodeURIComponent(it.id)}">
        <div class="recent-file-top">
          <span class="recent-file-name">${escapeHtml(it.clientName || it.fileName)}</span>
          <span class="badge-status" style="background:${statusColor[it.status] || "#6b7280"}">${statusLabel[it.status] || it.status}</span>
        </div>
        <div class="recent-file-meta">
          <span>${escapeHtml(it.fileName)}</span>
          <span>${it.facilitiesCount} تسهيل</span>
          <span>${new Date(it.analyzedAt).toLocaleDateString("ar-EG-u-nu-latn")}</span>
        </div>
      </a>`).join("");
  } catch {
    wrap.innerHTML = `<div class="empty-state" style="padding:24px"><p>تعذر تحميل السجل المحلي.</p></div>`;
  }
}
