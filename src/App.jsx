...
export default function App() {
  ...
  const onUpload = async (e) => {
    ...
    try {
      ...
      if (processedSheets[0].validation.missingFields.length > 0) {
        setMappingSheet(processedSheets[0]);
      }
  ...
  const handleMap = (newMap) => {
    ...
    const newData = {...data};
    newData.sheets[0] = { ...s, colMap: newMap, validation: val, metrics: met };
    setData(newData);
        ...
        <div className="space-y-10">
          <div className="flex justify-between bg-white p-4 shadow rounded">
             <div className="text-sm">
                تم استيراد: {data.sheets[0].rows.length} صف | 
                صحيح: {data.sheets[0].validation.valid.length} | 
                ملخصات: {data.sheets[0].validation.summaries.length}
             </div>
