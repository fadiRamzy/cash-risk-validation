...
(function (global) {
  ...
  function fmtNumber(n) {
    ...
    try { return n.toLocaleString("ar-EG"); } catch { return String(n); }
  }
  ...
  function parseDetailed(rawText, lookup, diagnostics) {
    ...
    for (const part of parts) {
      try {
        ...
        const relationship = relKnown ? collapse(relKnown[1]).trim() : relMatch ? collapse(relMatch[1].replace(/يوجد/g, "")).trim() : "غير متاح";
      ...
      if (closedBlockMatch) {
        ...
        while ((m = re.exec(closedBlockMatch[1]))) {
          ...
          facilities.push({
            ...
            date: closeDate, amount, installment: "غير متاح", lastPayment: "غير متاح", relationship: relationship.trim(),
      ...
      if (activeBlockMatch) {
        ...
        while ((m = re.exec(activeBlockMatch[1]))) {
          ...
          facilities.push({
            ...
            date: approvalDate, amount, installment, lastPayment: "غير متاح", status: "ساري", relationship: relationship.trim(),
  ...
  function buildTimeline(facilities, diagnostics) {
    ...
    try {
      facilities.forEach((f) => {
        ...
        if (f.closeDate) {
          const cDate = parseArabicDate(f.closeDate);
          if (cDate) {
            events.push({
        ...
        if (typeof f.delay === "string" && f.delay.startsWith("متأخر") && f.overdueAmount && f.overdueAmount !== "غير متاح") {
          // نستخدم تاريخ المنح كمرساة زمنية تقريبية لعرض التأخير المرتبط بالتسهيل عندما لا يتوفر تاريخ محدد للتأخير
          const anchor = openDate;
          if (anchor) {
            events.push({
