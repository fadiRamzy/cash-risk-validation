...
function search(query, category) {
  ...
  if (!nQuery) {
    ...
    for (const r of pool) {
      ...
      else if (fuzzyContains(r.nName, nQuery) || fuzzyContains(r.nCode, nQuery)) score = 30;
...
// lightweight fuzzy: all characters of query appear in order within target
...
function exportCsv() {
  ...
  const { results } = search(q, state.activeCategory);
  const rows = [["الاسم", "الكود", "التصنيف"], ...results.map((r) => [r.name, r.code, r.category])];
  const csv = rows.map((r) => r.map((c) => `"${(c || "").replace(/"/g, '""')}"`).join(",")).join("\n");
  downloadBlob(new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8" }), "نتائج-البحث.csv");
  toast("تم تصدير النتائج CSV");
}
function exportPdf() {
  const q = $("#search-input").value;
  const { results } = search(q, state.activeCategory);
  ...
  const rowsHtml = results
    .map((r) => `<tr><td>${escapeHtml(r.name)}</td><td>${escapeHtml(r.code)}</td><td>${escapeHtml(r.category)}</td></tr>`)
    .join("");
  win.document.write(`
    <html dir="rtl"><head><meta charset="utf-8"><title>نتائج البحث</title>
    <style>
    ...
    <h2>أكواد الاستعلامات الائتمانية — نتائج البحث</h2>
    <table><thead><tr><th>الاسم</th><th>الكود</th><th>التصنيف</th></tr></thead><tbody>${rowsHtml}</tbody></table>
